...
export function BillItemsManager() {
  ...
  const handleItemChange = (id: string, field: keyof BillItem, value: string | number) => {
    ...
    if (item) {
      const updatedItem = { ...item, [field]: field === 'price' ? parseFloat(value as string) || 0 : value };
      dispatch({ type: 'UPDATE_ITEM', payload: updatedItem });
    }
  };

  const handleBillDetailChange = (field: keyof typeof state.billDetails, value: string) => {
    dispatch({ type: 'UPDATE_BILL_DETAILS', payload: { [field]: parseFloat(value) || 0 } });
  };

  const addNewItem = () => {
    dispatch({ type: 'ADD_ITEM', payload: { name: 'New Item', price: 0 } });
  };
  ...
  if (!state.isOcrCompleted && state.items.length === 0) {
    return (
      <Card className="shadow-lg">
        ...
        <CardContent>
          <p className="text-muted-foreground">Upload a bill and extract items to manage them here.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <Card className="shadow-lg">
      <CardHeader>
        ...
        <CardDescription>Review and edit extracted items, or add them manually. Adjust bill totals if needed.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <h3 className="text-md font-semibold">Items</h3>
        <ScrollArea className="h-64 pr-3">
          {state.items.length === 0 ? (
            <p className="text-muted-foreground">No items yet. Add items manually or extract them from a bill.</p>
          ) : (
            <ul className="space-y-3">
              {state.items.map((item) => (
                <li key={item.id} className="flex items-center gap-2 p-2 border rounded-md bg-background">
                  <Input
                    type="text"
