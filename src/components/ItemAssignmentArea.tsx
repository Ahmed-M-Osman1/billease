...
function DraggableItem({item}: {item: BillItem}) {
  ...
  return (
    <div
      ...
      aria-label={`Item ${item.name}, price ${item.price.toFixed(
        2
      )}`}>
...
export function ItemAssignmentArea() {
  ...
  const [selectedItem, setSelectedItem] = useState<BillItem | null>(
    null
  );
  ...
  const handleAssignItem = (targetId: string | null) => {
    if (selectedItem) {
      dispatch({
        type: 'ASSIGN_ITEM',
        payload: {itemId: selectedItem.id, targetId},
      });
    }
      ...
      <Card className="shadow-lg">
        ...
        <CardContent>
          <p className="text-muted-foreground">
            Add or extract bill items to start.
          </p>
        </CardContent>
      </Card>
    );
    ...
    <div className="space-y-6">
      <Card className="shadow-lg border-primary/50">
        <CardHeader>
          ...
          <CardDescription>
            {isMobile
              ? 'Tap an item to assign it.'
              : 'Drag items to a person or group.'}{' '}
            Items with a price of 0 are not shown.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isMobile ? (
      ...
      {state.error && state.error.includes('Suggestion') && (
      ...
      <Dialog
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Select a person or group to assign this item to.
            </DialogDescription>
          </DialogHeader>
          <ScrollArea className="max-h-[60vh]">
            <div className="p-1 space-y-2">
              ...
              <Button
                variant="ghost"
                className="w-full justify-start"
                onClick={() => handleAssignItem('SHARED_ALL_PEOPLE')}>
              ...
              {state.customSharedPools.map((pool) => (
                <Button
                  ...
                  variant="ghost"
                  className="w-full justify-start"
                  onClick={() => handleAssignItem(pool.id)}>
              ...
              {state.people.map((person) => (
                <Button
                  ...
                  variant="ghost"
                  className="w-full justify-start"
                  onClick={() => handleAssignItem(person.id)}>
                  <User className="mr-2" />
                  {person.name}
                </Button>
              ))}
...
function DesktopLayout({items}: {items: BillItem[]}) {
  ...
  const sharedAllPeopleItems = items.filter(
    ...
    return (
      <Card
        className={`flex-1 transition-all ${
          isOver ? 'bg-accent/30 ring-2 ring-primary' : 'bg-muted/20'
        }`}
        ...
        <CardHeader className="pb-2 pt-4">
          ...
          {subtitle && (
            <CardDescription className="text-xs -mt-1 ml-7">
              {subtitle}
            </CardDescription>
          )}
        </CardHeader>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {state.customSharedPools.map((pool) => (
              <DropZone
                ...
                targetId={pool.id}
                title={pool.name}
                subtitle={`With: ${pool.personIds
                  .map(
                    (pid) =>
                      state.people.find((p) => p.id === pid)?.name
                  )
                  .join(', ')}`}
                items={items.filter(
...
function MobileLayout({
  ...
  items: BillItem[];
  onAssignClick: (item: BillItem) => void;
}) {
  const {state} = useBillContext();
  ...
  const getAssignedToName = (assignedTo: string | null) => {
    ...
    if (assignedTo === 'SHARED_ALL_PEOPLE')
      return (
    ...
    const pool = state.customSharedPools.find(
      (p) => p.id === assignedTo
    );
    if (pool)
      return (
          ...
          items.map((item) => (
            <div key={item.id} className="mb-2">
              ...
              <div className="text-xs pr-2">
                Assigned to: {getAssignedToName(item.assignedTo)}
              </div>
            </div>
          ))
        ) : (
          <p className="text-muted-foreground text-center py-8">
            No items with a price greater than 0 to assign.
          </p>
        )}
      </ScrollArea>
    </div>
