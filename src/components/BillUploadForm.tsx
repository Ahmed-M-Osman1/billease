...
export function BillUploadForm() {
  ...
  const handleCapture = (dataUri: string) => {
    dispatch({
      type: 'SET_BILL_IMAGE',
      payload: {name: `capture-${Date.now()}.jpg`, dataUri},
    });
  ...
  const handleExtractItems = async () => {
    if (!state.billImageDataUri) {
      toast({
        ...
        description: 'Please upload or scan a bill image first.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : 'Unknown error during OCR';
      dispatch({
        type: 'OCR_FAILURE',
        payload: `Failed to extract items: ${errorMessage}`,
      });
      toast({
        title: 'OCR Failed',
        description: `Could not extract items. ${errorMessage}`,
        variant: 'destructive',
      });
    }
  };
  ...
  return (
    <Card className="shadow-lg">
      <CardHeader>
        ...
        <CardDescription>
          Upload a photo or scan your bill to get started.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div>
          ...
          <Label htmlFor="bill-image" className="sr-only">
            Bill Image
          </Label>
          <Input
            id="bill-image"
            ...
            className="mt-4 file:text-sm file:font-medium file:text-primary-foreground file:bg-primary hover:file:bg-primary/90 file:rounded-md file:px-3 file:py-1 file:border-0"
        ...
        {state.billImageName && (
          <div
            ...
            className="text-sm text-muted-foreground flex items-center justify-between p-2 border rounded-md">
        ...
        <div className="flex items-center space-x-3 my-3 p-2 border rounded-md bg-muted/30">
          <Switch
            ...
            onCheckedChange={(checked) => {
              dispatch({
                type: 'SET_OCR_PRICE_MODE',
                payload: checked ? 'unit' : 'total',
              });
            }}
            aria-label="OCR Price Interpretation Mode Toggle"
          />
          <Label
            htmlFor="ocr-price-mode"
            className="text-sm text-foreground/90 leading-tight">
        ...
        <Button
          ...
          disabled={!state.billImageDataUri || state.isLoadingOCR}
          className="w-full">
          {state.isLoadingOCR ? (
        ...
        {state.error && state.error.includes('OCR') && (
