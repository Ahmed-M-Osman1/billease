...
type BillAction =
  ...
  | {
      type: 'ASSIGN_ITEM';
      payload: {itemId: string; targetId: string | null};
    } // targetId can be personId, customPoolId, 'SHARED_ALL_PEOPLE' or null
  | {type: 'RESET_ASSIGNMENTS'}
  | {type: 'START_SUGGESTION'}
  | {
      type: 'SUGGESTION_SUCCESS';
      payload: {assignments: Record<string, string>};
    } // { itemName: personName }
  ...
  | {
      type: 'ADD_CUSTOM_SHARED_POOL';
      payload: {name: string; personIds: string[]};
    }
...
function billReducer(
  ...
  switch (action.type) {
    case 'SET_BILL_IMAGE':
      return {
        ...state,
        billImageName: action.payload.name,
        billImageDataUri: action.payload.dataUri,
        isOcrCompleted: false,
        items: [],
    ...
    case 'OCR_SUCCESS': {
      ...
      action.payload.items.forEach((itemFromOcr) => {
        ...
        if (
          state.ocrPriceMode === 'total' &&
          quantity > 1 &&
          itemFromOcr.price > 0
        ) {
          unitPrice = parseFloat(
            (itemFromOcr.price / quantity).toFixed(2)
          );
        }
      ...
      return {
        ...
        billDetails: {
          ...
          delivery: action.payload.delivery ?? 0,
        },
        error: null,
      };
    }
    ...
    case 'SET_PEOPLE_COUNT': {
      ...
      let updatedCustomSharedPools = state.customSharedPools
        .map((pool) => ({
          ...pool,
          personIds: pool.personIds.filter((pid) =>
            newPeopleIds.has(pid)
          ),
        }))
        .filter((pool) => pool.personIds.length > 0); // Remove pools with no people

      // Unassign items from deleted custom shared pools
      const remainingCustomPoolIds = new Set(
        updatedCustomSharedPools.map((p) => p.id)
      );
      updatedItems = updatedItems.map((item) => {
        if (
          ...
          item.assignedTo !== 'SHARED_ALL_PEOPLE' &&
          !newPeopleIds.has(item.assignedTo) &&
          !remainingCustomPoolIds.has(item.assignedTo)
        ) {
          return {...item, assignedTo: null};
        }
        return item;
      ...
      return {
        ...
        people: newPeople,
        items: updatedItems,
        customSharedPools: updatedCustomSharedPools,
      };
    }
      ...
      return {
        ...
        items: state.items.map((item) =>
          item.id === action.payload.itemId
            ? {...item, assignedTo: action.payload.targetId}
            : item
        ),
      };
    ...
    case 'ADD_CUSTOM_SHARED_POOL': {
      ...
      localStorage.setItem(
        'billEaseCustomSharedPools',
        JSON.stringify(updatedPools)
      );
      return {...state, customSharedPools: updatedPools};
    }
    case 'UPDATE_CUSTOM_SHARED_POOL': {
      const updatedPools = state.customSharedPools.map((pool) =>
    ...
    case 'DELETE_CUSTOM_SHARED_POOL': {
      ...
      const updatedPools = state.customSharedPools.filter(
        (pool) => pool.id !== poolIdToDelete
      );
      localStorage.setItem(
        'billEaseCustomSharedPools',
        JSON.stringify(updatedPools)
      ...
      const updatedItems = state.items.map((item) =>
        item.assignedTo === poolIdToDelete
...
const BillContext = createContext<
  {state: BillState; dispatch: Dispatch<BillAction>} | undefined
>(undefined);
...
export const BillProvider = ({children}: {children: ReactNode}) => {
  ...
  useEffect(() => {
    ...
    const storedCustomPoolsRaw = localStorage.getItem(
      'billEaseCustomSharedPools'
    );
    if (storedCustomPoolsRaw) {
      try {
        const storedCustomPools = JSON.parse(
          storedCustomPoolsRaw
        ) as CustomSharedPool[];
        if (
          Array.isArray(storedCustomPools) &&
          storedCustomPools.every(
            (p) =>
              typeof p.id === 'string' &&
              typeof p.name === 'string' &&
              Array.isArray(p.personIds)
          )
        ) {
          dispatch({
            type: 'LOAD_CUSTOM_SHARED_POOLS',
            payload: storedCustomPools,
          });
        } else {
          localStorage.removeItem('billEaseCustomSharedPools');
        }
...
export const useBillContext = () => {
  ...
  if (context === undefined) {
    throw new Error(
      'useBillContext must be used within a BillProvider'
    );
  }
  return context;
};
